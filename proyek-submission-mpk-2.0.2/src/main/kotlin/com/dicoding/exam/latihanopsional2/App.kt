...
fun minAndMax(number: Int): Int {
    val digits = generateSequence(number) { it / 10 }
        .takeWhile { it > 0 }
        .map { it % 10 }
        .toList()
    ...
    val (minValue, maxValue) = digits.fold(Pair(digits[0], digits[0])) { (min, max), digit ->
        Pair(min.coerceAtMost(digit), max.coerceAtLeast(digit))
    }
